          ...
          data.map((item) => {
            return (
              <li key={item.episode_id}>
                <h2>{item.title}</h2>
                <p style={{color:"#878787"}}>{new Date(item.release_date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>
                <p className="breakpoint">{item.opening_crawl.split('\n').slice(0,10).join('\n')}</p>
                <a href="#">More Info</a>
              </li>
            );
          })}
